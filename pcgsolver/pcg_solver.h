...
//cout<<endl<<max_iterations<<" "<<residual_out<<endl;
...
/*         cout<<"\nok\n";
      ...
      for(unsigned int i=0; i<matrix.n; ++i,cout<<"\n"){
		for(unsigned int j=0; j<matrix.index[i].size(); ++j){
         cout<<" "<<s[i];
      }
   }
      ...
      for(unsigned int i=0; i<matrix.n; ++i,cout<<"\n"){
        ...
        // cout<<endl<<residual_out<<" dot,rho =  "<<BLAS::dot(s, z)<<" "<<rho<<" "<<tol;
